...
- 训练方式为因果语言建模（Causal LM），不采用传统 SFT 配对，保留连发、跳跃等微信风格。
...
2. **max_length 设置**：微信聊天实际长度多在几百 token，1536 已足够，若显存不足可降至 1024。
...
本流程构建了一个完整的微信人格模型训练体系，包含环境配置、数据准备、训练、监控、测试和继续微调。关键在于数据构造方式符合微信聊天的自然流，而非简单 QA 对。建议在实际使用中根据数据特点灵活调整清洗参数和训练策略。标准 Causal LM 方式训练（prompt + response 都算 loss），而不是像 SFT 那样只对 response 算 loss。目的在于先注入风格在进行对话，因为大部分微信数据都是跳跃很大的。