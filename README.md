...
python process.py
```
脚本会遍历输入目录中的所有 `.cvs``.json` 和 `.jsonl` 文件，进行清洗、切分、合并，最终生成一个纯文本流 JSONL 文件（每行 `{"text": "..."}`），并输出统计信息。

**注意**：这里的清洗方式和weclone不同。本项目构造的是多轮对话，而非简单 QA 问答。微信聊天本身断断续续，因此采用的清洗逻辑更为激进。你可以根据实际需求调整清洗参数。
